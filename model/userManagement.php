...
function CheckRegister($formR)
{
    ...
    $pseudo = $formR['pseudo'];
    $type= $formR ['type'];      // le pseudo du formulaire dans la variable pseudo
    ...
    if ($queryResult) {
        ...
        if ($formR['passwordRegister'] == $formR['passwordVerify']) {    // si le password du formulaire register est identique  avec la confirmation du mdp, hash le mdp.
            ...
            $requeteAdd = "INSERT INTO snows.users (userEmailAddress, userPsw, pseudo, types) VALUES ('$email', '$pwdHash','$pseudo','$type');";       // ajoute le compte dans la base de donnée
